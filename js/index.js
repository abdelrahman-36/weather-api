...
async function getResponse( x = `cairo`) {
 var response = await fetch(
    `http://api.weatherapi.com/v1/forecast.json?key=12c1b05d64854f8fa7a163415240312&q=${x}&days=3`
  );
  ...
  function displayCurrent(data) {
    ...
    d = new Date(data.current.last_updated);
     ...
     cartoona = `<div class="col-md-4  border border-secondary ">
...
`;
  ...
  function displayForecast(data) {
    ...
    for (let i = 1; i < data.length; i++) {
      cartone += `
      <div class="col-md-4 text-center border border-secondary ">
